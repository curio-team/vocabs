...
{
  $hasEnglishTranslation = isset($vocab['basic'][0]) ? isset($vocab['basic'][0]['eng']) : false;
  ...
  if ($hasEnglishTranslation)
    $html .= '<li class="header basic">Basic</li>';

  foreach ($vocab['basic'] as $token) {
    $html .= '<li><a class="vocab-token ' . $token['name'] . '" href="' . $token['url'] . '"><span class="nl">' . $token['text'] . '</span><span class="eng">' . (isset($token['eng']) ? $token['eng'] : '') . '</span></a></li>';
  }
...
$vocabName = isset($_GET['name']) ? htmlspecialchars($_GET['name']) : false;
$locale = isset($_GET['lang']) ? htmlspecialchars($_GET['lang']) : false;
if (!$vocabName && !$locale) {
  include('list.php');
  exit();
}
